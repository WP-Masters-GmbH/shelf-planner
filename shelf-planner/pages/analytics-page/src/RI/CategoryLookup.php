...
class CategoryLookup
{
    ...
    public function regenerate_category_lookup_table()
    {
        ...
        $wpdb->query("INSERT IGNORE INTO {$this->table_name} (category_tree_id,category_id) VALUES ({$insert_string})"); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared
    }
    ...
    protected function delete($category_id, $category_tree_id)
    {
        ...
        foreach ($ancestors as $ancestor) {
            $wpdb->query($wpdb->prepare("DELETE FROM {$this->table_name} WHERE category_tree_id = %d AND category_id IN ({$id_list})", $ancestor)); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared
        }
    }
    ...
    protected function update($category_id)
    {
        ...
        $wpdb->query("INSERT IGNORE INTO {$this->table_name} (category_id, category_tree_id) VALUES {$insert_string}"); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared
    }
