...
namespace QuickAssortmentsSP\COG\RI\API\Reports\Products\Stats;

defined('ABSPATH') || exit;

use QuickAssortmentsSP\COG\RI\API\Reports\ParameterExceptionSP;
use QuickAssortmentsSP\COG\RI\API\Reports\SegmenterSP as ReportsSegmenter;

/**
 * Date & time interval and numeric range handling class for Reporting API.
 */
class SegmenterSP extends ReportsSegmenter
{
    /**
     ...
     *
     * @return array
     *@throws \QuickAssortmentsSP\COG\RI\API\Reports\ParameterExceptionSP In case of segmenting by variations, when no parent product is specified.
     *
     */
    protected function get_segments($type, $query_params, $table_name)
    {
        ...
        $product_segmenting_table = $wpdb->prefix . 'sp_cog_order_product_lookup';
        ...
        if ('product' === $this->query_args['segmentby']) {
            ...
            if (! isset($this->query_args['product_includes']) || count($this->query_args['product_includes']) !== 1) {
                throw new ParameterExceptionSP('wc_admin_reports_invalid_segmenting_variation', __('product_includes parameter need to specify exactly one product when segmenting by variation.', 'qa-cost-of-goods-margins'));
            }
			...
			LEFT JOIN {$wpdb->prefix}sp_cog_category_lookup ON {$wpdb->term_relationships}.term_taxonomy_id = {$wpdb->prefix}sp_cog_category_lookup.category_id
			";
            $segmenting_where          = " AND {$wpdb->prefix}sp_cog_category_lookup.category_tree_id IS NOT NULL";
            $segmenting_groupby        = "{$wpdb->prefix}sp_cog_category_lookup.category_tree_id";
            ...
            if (isset($this->query_args['categories'])) {
                ...
                $segmenting_where .= " AND {$wpdb->prefix}sp_cog_category_lookup.category_id IN ( $category_ids )";
            }
