...
use QuickAssortments\COG\RI\API\Reports\Cache as ReportsCache;
...
final class DataSync
{
    ...
    public function update($order)
    {
         ...
         * @param int $order_id Order ID.
         */
        do_action('qasss_cog_order_stats_table_update', $order->get_id());
    ...
    public function sync_order_products($order_id)
    {
        ...
        foreach ($order_items as $order_item) {
            ...
            $shipping_amount     = Helpers\Order::get_item_shipping_amount($order_item, $order);
            $shipping_tax_amount = Helpers\Order::get_item_shipping_tax_amount($order_item, $order);
            $coupon_amount       = Helpers\Order::get_item_coupon_amount($order_item);
            ...
            if ($variation_id && $variation_id > 0) {
                $product_cost = (float) Helpers\Methods::get_cost($variation_id);
            } else {
                $product_cost = (float) Helpers\Methods::get_cost($product_id);
            }
            ...
            $data   = [
                ...
                'product_cost'          => $product_cost = ($product_cost * $product_qty),
                'product_net_profit'    => $net_profit = ($net_revenue - $product_cost),
                'product_margin_ex_tax' => Helpers\Formulae::margin($product_cost, $net_revenue) * $product_qty,
                'product_margin_in_tax' => Helpers\Formulae::margin($product_cost, ($net_revenue + $tax_amount)) * $product_qty,
            ];
            $format = [
                '%d', // order_item_id.
                '%d', // order_id.
            ...
            if ('shop_order_refund' === $order->get_type() && $order->get_parent_id() > 0) {
                ...
                $data['product_margin']     = -1 * abs(Helpers\Formulae::margin($data['product_cost'], $net_revenue));
            }

            $result = $wpdb->replace($wpdb->prefix . 'sp_cog_order_product_lookup', $data, $format); // WPCS: cache ok, DB call ok, unprepared SQL ok.
    ...
    public function delete_order($post_id)
    {
         ...
         * @param int $order_id Order ID.
         */
        do_action('qasss_cog_order_stats_table_delete_order_stats', $order_id);
