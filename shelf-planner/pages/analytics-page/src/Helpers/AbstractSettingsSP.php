...
/**
 * Class AbstractSettingsSP.
 *
 * @package QuickAssortmentsSP\COG\Helpers
 *
 * @since   1.0.0
 */
abstract class AbstractSettingsSP
{
    /**
    ...
    public function output_sections()
    {
        ...
        foreach ($sections as $id => $label) {
            echo '<li><a href="' . esc_url( admin_url('admin.php?page=' . self::$nonce_key . '&tab=' . $this->id . '&section=' . sanitize_title($id)) ) . '" class="' . ($current_section == $id ? 'current' : '') . '">' . $label . '</a> ' . (end($array_keys) == $id ? '' : '|') . ' </li>';
        }
