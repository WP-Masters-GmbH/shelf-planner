...
require_once QA_COG_BASE_PATHSP.'libs/SimpleXLSX/SimpleXLSXSP.php';
require_once QA_COG_BASE_PATHSP.'libs/SimpleXLSXGen/SimpleXLSXGenSP.php';

use QuickAssortmentsSP\COG\Helpers\AbstractSettingsSP;
 ...
 *
 * @package  QuickAssortmentsSP\COG\Premium\SettingsSPSP
 *
 * @since    1.0.0
 */
class SettingsSPSP extends AbstractSettingsSP
{
    /**
     * SettingsSPSP constructor.
     */
    public function __construct()
    {
        $this->id              = 'qa_cog_main_settings_sp';
        $this->label           = __('Cost of Goods & Margins Settings SP', 'qa-cost-of-goods-margins');
    ...
    public function import_qa_costs()
    {
        if(isset($_POST) && isset($_FILES['file'])) {
            ...
            if($xlsx = \QASimpleXLSXSP::parse($_FILES['file']['tmp_name'])) {
    ...
    public function export_qa_costs()
    {
        if(isset($_GET['export_qa_products'])) {
            ...
            // Export Data
            $table = \SimpleXLSXGenSP::fromArray($data)->downloadAs('export-qa-costs.xlsx');
        ...
        switch ($section) {
            case $this->id:
                $settings = apply_filters('qa_cog_main_' . $section . '_body', [
                    [
                        'title' => __('Product SettingsSPSP', 'qa-cost-of-goods-margins'),
                        'type'  => 'title',
