...
class XLSXWriter
{
    ...
    protected function tempFilename()
    {
        ...
        if (!$filename) {
	        $filename = @tempnam( SP_ROOT_DIR . '/tmp', "xlsx_writer_" );
	        if ( ! $filename ) {
    ...
    protected function writeStylesXML()
    {
        ...
        foreach ($fonts as $font) {
            if (!empty($font)) {
                //fonts have 4 empty placeholders in array to offset the 4 static xml entries above
